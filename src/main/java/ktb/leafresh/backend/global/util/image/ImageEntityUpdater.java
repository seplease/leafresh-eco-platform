...
public class ImageEntityUpdater {
  ...
  private <E extends BaseEntity & ImageEntity> void handleDeletes(
      List<Long> deletedIds, JpaRepository<E, Long> repository) {
    if (deletedIds != null) {
      deletedIds.forEach(id -> repository.findById(id).ifPresent(BaseEntity::softDelete));
    }
  }

  private <E extends BaseEntity & ImageEntity> void handleUpdates(
      List<KeepImage> keepList, JpaRepository<E, Long> repository) {
    if (keepList != null) {
  ...
  private <E extends BaseEntity & ImageEntity> void handleInserts(
      List<E> newEntities, JpaRepository<E, Long> repository) {
