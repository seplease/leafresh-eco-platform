...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ApiResponse<String>> handleAuthenticationException(Exception ex) {
    ...
    return ResponseEntity.status(GlobalErrorCode.INVALID_TOKEN.getStatus())
        .body(
            ApiResponse.error(
                GlobalErrorCode.INVALID_TOKEN.getStatus(),
                GlobalErrorCode.INVALID_TOKEN.getMessage()));
  }
  ...
  public ResponseEntity<ApiResponse<String>> handleAccessDeniedException(AccessDeniedException ex) {
    ...
    return ResponseEntity.status(GlobalErrorCode.ACCESS_DENIED.getStatus())
        .body(
            ApiResponse.error(
                GlobalErrorCode.ACCESS_DENIED.getStatus(),
                GlobalErrorCode.ACCESS_DENIED.getMessage()));
  }

  /** JSON 파싱 실패 예외 처리 */
  @ExceptionHandler(HttpMessageNotReadableException.class)
  public ResponseEntity<ApiResponse<String>> handleParseException(
    ...
    return ResponseEntity.status(GlobalErrorCode.INVALID_JSON_FORMAT.getStatus())
        .body(
            ApiResponse.error(
                GlobalErrorCode.INVALID_JSON_FORMAT.getStatus(),
                GlobalErrorCode.INVALID_JSON_FORMAT.getMessage()));
  }

  /** 정적 리소스가 존재하지 않을 때 (ex. favicon.ico 요청 실패) - 에러 로그로 남기지 않고 404만 응답 */
  ...
  public ResponseEntity<ApiResponse<String>> handleException(Exception ex) {
    ...
    return ResponseEntity.status(GlobalErrorCode.NOT_FOUND.getStatus())
        .body(
            ApiResponse.error(
                GlobalErrorCode.NOT_FOUND.getStatus(), GlobalErrorCode.NOT_FOUND.getMessage()));
  }
}
