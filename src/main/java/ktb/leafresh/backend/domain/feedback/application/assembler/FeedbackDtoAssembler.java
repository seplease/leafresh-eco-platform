...
public class FeedbackDtoAssembler {
  ...
  private List<AiFeedbackCreationRequestDto.PersonalChallengeDto> getPersonalChallengeDtos(
    ...
    return verifications.stream()
        ...
        .map(
            entry -> {
              ...
              boolean isSuccess =
                  entry.getValue().stream().anyMatch(v -> v.getStatus() == ChallengeStatus.SUCCESS);
  ...
  private List<AiFeedbackCreationRequestDto.GroupChallengeDto> getGroupChallengeDtos(
      Long memberId, LocalDate start, LocalDate end) {
    List<GroupChallengeParticipantRecord> records =
        groupRecordRepository.findAllByMemberId(memberId);
    log.debug("[단체 참여 기록 조회] count={}", records.size());

    List<Long> recordIds = records.stream().map(GroupChallengeParticipantRecord::getId).toList();

    List<GroupChallengeVerification> allVerifications =
        groupVerificationRepository.findAllByParticipantRecordIds(recordIds);

    Map<Long, List<GroupChallengeVerification>> groupedVerifications =
    ...
    for (GroupChallengeParticipantRecord record : records) {
      List<GroupChallengeVerification> verifications =
          groupedVerifications.getOrDefault(record.getId(), List.of());

      List<AiFeedbackCreationRequestDto.GroupChallengeDto.SubmissionDto> submissions =
  ...
  private AiFeedbackCreationRequestDto.GroupChallengeDto.SubmissionDto toSubmissionDto(
