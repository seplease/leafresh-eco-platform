...
public class VerificationResultProcessor {
  ...
  public void process(Long verificationId, VerificationResultRequestDto dto) {
    if (!dto.isSuccessResult()) {
      log.warn(
          "[Processor 인증 결과 무시] 정상적인 true/false 값이 아님: result={}, type={}",
          dto.result(),
          dto.type());
      return;
    }
  ...
  private void processGroup(Long verificationId, VerificationResultRequestDto dto) {
    ...
    notificationCreateService.createChallengeVerificationResultNotification(
        ...
        challenge.getId());
    log.info(
        "[Processor 알림 생성 완료] memberId={}, challengeTitle={}",
        member.getId(),
        challenge.getTitle());
  ...
  private void processPersonal(Long verificationId, VerificationResultRequestDto dto) {
    boolean isSuccess = dto.resultAsBoolean();
    log.info(
        "[Processor 개인 인증 결과 수신] verificationId={}, type={}, result={}",
        verificationId,
        dto.type(),
        isSuccess);

    PersonalChallengeVerification verification =
        personalChallengeVerificationRepository
            .findById(verificationId)
            .orElseThrow(
                () -> {
                  log.error(
                      "[Processor 인증 결과 저장 실패] verificationId={}에 해당하는 인증이 존재하지 않음",
                      verificationId);
    ...
    verification.markVerified(newStatus);
    log.info(
        "[Processor 인증 상태 업데이트 완료] verificationId={}, newStatus={}", verificationId, newStatus);
    ...
    if (isSuccess) {
      if (verification.isRewarded()) {
        log.warn(
            "[Processor 보상 스킵] 이미 보상된 인증입니다. verificationId={}, memberId={}",
            verificationId,
            member.getId());
      } else {
        ...
        verification.markRewarded();
        log.info(
            "[Processor 보상 지급 완료] verificationId={}, memberId={}, reward={}",
            verificationId,
            member.getId(),
            reward);
      }
