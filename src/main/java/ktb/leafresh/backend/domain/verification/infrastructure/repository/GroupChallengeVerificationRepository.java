...
public interface GroupChallengeVerificationRepository
    extends JpaRepository<GroupChallengeVerification, Long> {

  /** 특정 회원이 특정 단체 챌린지에 대해 마지막으로 인증한 기록을 조회 */
  Optional<GroupChallengeVerification>
  ...
  List<GroupChallengeVerification>
      findTop9ByParticipantRecord_GroupChallenge_IdOrderByCreatedAtDesc(Long challengeId);
  ...
  @Query(
    ...
    """)
  List<GroupChallengeVerification> findAllByParticipantRecordIds(
  ...
  @Modifying
  @Query(
      "UPDATE GroupChallengeVerification g SET "
  ...
  @Query(
      "SELECT new ktb.leafresh.backend.domain.verification.application.dto.VerificationStatSnapshot("
