...
public class TimedealProductQueryRepositoryImpl implements TimedealProductQueryRepository {
  ...
  @Override
  public List<TimedealProductSummaryResponseDto> findTimedeals(
      LocalDateTime now, LocalDateTime oneWeekLater) {
    return queryFactory
        .select(
            Projections.constructor(
                TimedealProductSummaryResponseDto.class,
                tp.id,
                p.id,
                p.name, // title
                p.description,
                ...
                getTimeDealStatusCase(
                    now) // CASE WHEN start < now AND end > now THEN ONGOING ELSE UPCOMING
                ))
        .from(tp)
        .join(tp.product, p)
        .where(
  ...
  private Expression<String> getProductStatusCase() {
    return new CaseBuilder()
        .when(p.status.eq(ProductStatus.SOLD_OUT))
        .then("SOLD_OUT")
        .otherwise("ACTIVE");
  }
  ...
  private Expression<String> getTimeDealStatusCase(LocalDateTime now) {
    return new CaseBuilder()
        .when(tp.startTime.before(now).and(tp.endTime.after(now)))
        .then("ONGOING")
        .otherwise("UPCOMING");
  }
