...
public class ProductCacheService {
  ...
  public void updateSingleProductCache(Product product) {
    ...
    log.info(
        "[ProductCacheService] 일반 상품 ZSet 동기화 - productId={}, score={}", product.getId(), score);
  }
  ...
  public void cacheProductStock(Long productId, Integer stock) {
    ...
    log.info(
        "[ProductCacheService] 일반 상품 재고 캐시 저장 - key={}, stock={}, TTL={}초",
        key,
        stock,
        ttl.getSeconds());
  }
  ...
  public void cacheTimedealStock(Long policyId, Integer stock, LocalDateTime endTime) {
    ...
    if (ttl != null) {
      redisTemplate.opsForValue().set(key, stock, ttl);
      log.info(
          "[ProductCacheService] 타임딜 재고 캐시 저장 - key={}, stock={}, TTL={}초",
          key,
          stock,
          ttl.getSeconds());
    } else {
