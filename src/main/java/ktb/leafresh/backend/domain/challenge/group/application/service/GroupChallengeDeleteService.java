...
public class GroupChallengeDeleteService {
  ...
  public Long delete(Long memberId, Long challengeId) {
    ...
    if (!challenge.getMember().getId().equals(memberId)) {
      log.warn(
          "단체 챌린지 삭제 실패 - 삭제 권한 없음: 요청자 ID={}, 작성자 ID={}", memberId, challenge.getMember().getId());
      throw new CustomException(ChallengeErrorCode.CHALLENGE_ACCESS_DENIED);
    }

    boolean hasParticipants =
        participantRecordRepository.existsByGroupChallengeIdAndDeletedAtIsNull(challengeId);
    if (hasParticipants) {
    ...
    challenge
        ...
        .forEach(
            image -> {
              image.softDelete();
              log.debug(
                  "예시 이미지 soft delete - imageId: {}, imageUrl: {}",
                  image.getId(),
                  image.getImageUrl());
            });
