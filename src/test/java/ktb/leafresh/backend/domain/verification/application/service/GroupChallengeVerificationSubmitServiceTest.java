...
class GroupChallengeVerificationSubmitServiceTest {
  ...
  @Mock private GcpAiVerificationPubSubPublisher pubSubPublisher;

  @InjectMocks private GroupChallengeVerificationSubmitService submitService;
  ...
  void submit_success() {
    ...
    given(
            recordRepository.findByGroupChallengeIdAndMemberIdAndDeletedAtIsNull(
    ...
    then(recordRepository)
        .should()
        .findByGroupChallengeIdAndMemberIdAndDeletedAtIsNull(challengeId, memberId);
    then(verificationRepository).should().save(any(GroupChallengeVerification.class));
