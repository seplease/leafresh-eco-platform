...
class GroupVerificationCommentUpdateServiceTest {
  ...
  @InjectMocks private GroupVerificationCommentUpdateService updateService;
  ...
  void updateComment_withValidInput_returnsUpdatedResponse() {
    ...
    Comment comment =
        Comment.builder().verification(verification).member(member).content("기존 댓글").build();
    ...
    GroupVerificationCommentCreateRequestDto dto =
        new GroupVerificationCommentCreateRequestDto(newContent);
    CommentUpdateResponseDto result =
        updateService.updateComment(challengeId, verificationId, commentId, memberId, dto);
  ...
  void updateComment_withInvalidVerification_throwsException() {
    ...
    assertThatThrownBy(
            () ->
                updateService.updateComment(
                    1L,
                    verificationId,
                    100L,
                    999L,
                    new GroupVerificationCommentCreateRequestDto("수정")))
  ...
  void updateComment_notMine_throwsAccessDenied() {
    ...
    Comment comment =
        Comment.builder().verification(verification).member(other).content("다른 사람 댓글").build();
    ReflectionTestUtils.setField(comment, "id", 100L);

    given(verificationRepository.findByIdAndDeletedAtIsNull(10L))
        .willReturn(Optional.of(verification));
    ...
    assertThatThrownBy(
            () ->
                updateService.updateComment(
                    1L, 10L, 100L, memberId, new GroupVerificationCommentCreateRequestDto("수정")))
        .isInstanceOf(CustomException.class)
  ...
  void updateComment_withDeletedComment_throwsException() {
    ...
    Comment comment =
        Comment.builder().verification(verification).member(member).content("삭제된 댓글").build();
    ReflectionTestUtils.setField(comment, "id", 100L);
  ...
  void updateComment_whenUnexpectedExceptionThrown_throwsCreateFailed() {
    ...
    given(verificationRepository.findByIdAndDeletedAtIsNull(10L))
        .willReturn(Optional.of(verification));
    given(commentRepository.findById(100L)).willReturn(Optional.of(comment));
    given(commentRepository.findByParentCommentAndDeletedAtIsNull(comment))
