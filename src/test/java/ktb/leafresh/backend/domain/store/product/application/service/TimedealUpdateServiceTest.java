...
class TimedealUpdateServiceTest {
  ...
  @Mock private TimedealPolicyRepository policyRepository;

  @Mock private ProductCacheLockFacade productCacheLockFacade;

  @Mock private ApplicationEventPublisher eventPublisher;

  @InjectMocks private TimedealUpdateService service;

  private static final OffsetDateTime FIXED_NOW =
      OffsetDateTime.of(2025, 7, 1, 12, 0, 0, 0, ZoneOffset.UTC);
  ...
  void updateTimedeal_success() {
    ...
    verify(productCacheLockFacade)
        .cacheTimedealStock(policy.getId(), dto.stock(), dto.endTime().toLocalDateTime());
  ...
  void updateTimedeal_fail_notFound() {
    ...
    CustomException exception =
        catchThrowableOfType(() -> service.update(invalidId, dto), CustomException.class);
  ...
  void updateTimedeal_fail_invalidTime() {
    ...
    CustomException exception =
        catchThrowableOfType(() -> service.update(policy.getId(), dto), CustomException.class);
  ...
  void updateTimedeal_fail_invalidFields() {
    ...
    CustomException ex1 =
        catchThrowableOfType(() -> service.update(policy.getId(), dto1), CustomException.class);
    ...
    CustomException ex2 =
        catchThrowableOfType(() -> service.update(policy.getId(), dto2), CustomException.class);
    ...
    CustomException ex3 =
        catchThrowableOfType(() -> service.update(policy.getId(), dto3), CustomException.class);
