...
class AiChallengePolicyValidatorTest {
  ...
  @Mock private AiChallengeValidationClient aiChallengeValidationClient;

  @Mock private GroupChallengeRepository groupChallengeRepository;

  @InjectMocks private AiChallengePolicyValidator aiChallengePolicyValidator;
        ...
        new GroupChallengeCreateRequestDto(
            ...
            List.of(
                new ExampleImageRequestDto(
                    "https://dummy.image/img.png", ExampleImageType.SUCCESS, "성공 예시", 1)));
  }
  ...
  class ValidateSuccess {
    ...
    void validate_withValidInput_shouldPass() {
      ...
      given(
              aiChallengeValidationClient.validateChallenge(
                  any(AiChallengeValidationRequestDto.class)))
  ...
  class ValidateRequiredFieldFailure {
    ...
    void validate_withNullMemberId_shouldThrowException() {
      assertThatThrownBy(() -> aiChallengePolicyValidator.validate(null, validRequest))
          ...
          .hasMessageContaining(
              ChallengeErrorCode.GROUP_CHALLENGE_VALIDATION_MISSING_MEMBER_ID.getMessage());
    }
    ...
    void validate_withNullTitle_shouldThrowException() {
      GroupChallengeCreateRequestDto dto =
          new GroupChallengeCreateRequestDto(
              ...
              validRequest.maxParticipantCount(),
              validRequest.thumbnailImageUrl(),
              validRequest.startDate(),
              validRequest.endDate(),
              validRequest.verificationStartTime(),
              validRequest.verificationEndTime(),
              validRequest.exampleImages());

      assertThatThrownBy(() -> aiChallengePolicyValidator.validate(1L, dto))
          .isInstanceOf(CustomException.class)
          .hasMessageContaining(
              ChallengeErrorCode.GROUP_CHALLENGE_VALIDATION_MISSING_TITLE.getMessage());
    }
    ...
    void validate_withNullStartDate_shouldThrowException() {
      ...
      assertThatThrownBy(() -> aiChallengePolicyValidator.validate(1L, dto))
          .isInstanceOf(CustomException.class)
          .hasMessageContaining(
              ChallengeErrorCode.GROUP_CHALLENGE_VALIDATION_MISSING_START_DATE.getMessage());
    }
    ...
    void validate_withNullEndDate_shouldThrowException() {
      ...
      assertThatThrownBy(() -> aiChallengePolicyValidator.validate(1L, dto))
          .isInstanceOf(CustomException.class)
          .hasMessageContaining(
              ChallengeErrorCode.GROUP_CHALLENGE_VALIDATION_MISSING_END_DATE.getMessage());
    }
  }
  ...
  class ValidateAiFailure {
    ...
    void validate_whenAiRejects_shouldThrowException() {
      ...
      assertThatThrownBy(() -> aiChallengePolicyValidator.validate(1L, validRequest))
          .isInstanceOf(CustomException.class)
          .hasMessageContaining(
              ChallengeErrorCode.GROUP_CHALLENGE_VALIDATION_PROCESSING_FAILED.getMessage());
    }

    @Test
    @DisplayName("예외 발생 시 AI 처리 실패 예외로 전환")
    void validate_whenClientThrows_shouldThrowProcessingFailed() {
      given(groupChallengeRepository.findAllValidAndOngoing(LocalDateTime.now()))
          .willReturn(List.of());
      given(aiChallengeValidationClient.validateChallenge(any()))
