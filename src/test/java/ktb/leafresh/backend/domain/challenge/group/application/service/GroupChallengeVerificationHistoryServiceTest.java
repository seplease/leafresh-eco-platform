...
class GroupChallengeVerificationHistoryServiceTest {
  ...
  @Mock private GroupChallengeParticipantRecordRepository recordRepository;

  @Mock private GroupChallengeVerificationQueryRepository verificationRepository;

  @Mock private GroupChallengeVerificationHistoryCalculator calculator;

  @InjectMocks private GroupChallengeVerificationHistoryService historyService;
  ...
  void getVerificationHistory_success() {
    ...
    var expectedResponse =
        GroupChallengeVerificationHistoryResponseDto.builder()
            .id(challengeId)
            .title("제로웨이스트 챌린지")
            .achievement(
                GroupChallengeVerificationHistoryResponseDto.AchievementDto.builder()
                    .success(1)
                    .failure(0)
                    .remaining(6)
                    .build())
    ...
    given(
            recordRepository.findByMemberIdAndGroupChallengeIdAndDeletedAtIsNull(
                memberId, challengeId))
        .willReturn(Optional.of(record));
    given(verificationRepository.findByParticipantRecordId(record.getId()))
  ...
  void getVerificationHistory_challengeNotFound() {
    ...
    assertThatThrownBy(() -> historyService.getVerificationHistory(memberId, challengeId))
        ...
        .hasMessageContaining(
            ChallengeErrorCode.GROUP_CHALLENGE_VERIFICATION_CHALLENGE_NOT_FOUND.getMessage());
  }
  ...
  void getVerificationHistory_participantNotFound() {
    ...
    assertThatThrownBy(() -> historyService.getVerificationHistory(memberId, challengeId))
        .isInstanceOf(CustomException.class)
        .hasMessageContaining(
            ChallengeErrorCode.GROUP_CHALLENGE_VERIFICATION_ACCESS_DENIED.getMessage());
  }
  ...
  void getVerificationHistory_unknownError() {
    ...
    assertThatThrownBy(() -> historyService.getVerificationHistory(memberId, challengeId))
        .isInstanceOf(CustomException.class)
        .hasMessageContaining(
            ChallengeErrorCode.GROUP_CHALLENGE_VERIFICATION_READ_FAILED.getMessage());
  }
}
